	...
	<view v-if="getRequestOK">
		...
		<view class="model">
			...
			<view class="info-user">
				<view class="info">
					<text>{{ newInfo.author }} {{ newInfo.created_at | timeFormate }} 发布
						{{ newInfo.views_count }}阅读</text>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
			// console.log(options)
			let res = await this.$u.api.getNewsInfo(options)
			// 使用正则表达式解析内容部分的不规则语义部分，输出可以解析的html结构
